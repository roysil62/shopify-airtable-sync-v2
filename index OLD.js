...
async function createShopifyProduct(accessToken, record) {
    ...
    const productData = {
        product: {
            ...
            vendor: record.get('Vendor') || 'COLBO',
            product_type: record.get('Product Type') || '',
            variants: [{
                title: record.get('Variant Title') || 'Default',
                price: record.get('Price') || '0.00',
                sku: record.get('SKU') || '',
                inventory_quantity: record.get('Inventory') || 0
            }]
        }
    };
    ...
    try {
        ...
        for (const record of records) {
            try {
                ...
                console.log(`✅ Published: ${record.get('Product Title')}`);
            } catch (error) {
                console.error(`❌ Failed to publish ${record.get('Product Title')}:`, error.message);
            }
        }
